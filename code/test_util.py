  ...
  for x in range(0, sx):
    ...
    for y in range(0, sy):
      ...
      for z in range(0, sz):
        ...
        with torch.no_grad():
          y1_dis, y1, _ = net(test_patch)
          y2, y2_seg, _ = net(test_patch)
          # ensemble
          y = torch.sigmoid(y1)
...
def calculate_metric_percase(pred, gt):
  ...
  if pred.sum() > 0 and gt.sum() > 0:
    ...
    hd95 = metric.binary.hd95(pred, gt)  # 95th percentile of the Hausdorff Distance.
    return np.array([dice, jaccard, asd, hd95])
  else:
    return np.zeros(4)
